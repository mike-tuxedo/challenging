class ReactiveStore {
        ...
        this.rs = new Proxy(this.store, {
            get(target, prop) {
                ...
                if (v && typeof v === 'object') {
                    // verschachtelte Objekte/Arrays als Deep‑Proxy zurückgeben
                    return self.wrapDeep(v, [prop]);
                }
                return v;
            },
    ...
    // ADDED: Deep-Proxy für verschachtelte Objekte/Arrays
    wrapDeep(obj, path = []) {
        if (!obj || typeof obj !== 'object') return obj;
        const self = this;

        const arrayMutators = ['push', 'pop', 'shift', 'unshift', 'splice', 'sort', 'reverse', 'copyWithin', 'fill'];

        return new Proxy(obj, {
            get(target, key, receiver) {
                const value = Reflect.get(target, key, receiver);

                // Array-Mutatoren abfangen, um Änderungen zu signalisieren
                if (Array.isArray(target) && typeof key === 'string' && arrayMutators.includes(key)) {
                    return function (...args) {
                        const res = Array.prototype[key].apply(target, args);
                        // Basis-Variable aktualisieren (z.B. "publicChallenges")
                        self.updateVariable(path[0], self.store[path[0]]);
                        self.emit('change', path[0], undefined, self.store[path[0]]);
                        return res;
                    };
                }

                // Verschachtelte Objekte/Arrays weiter wrappen
                if (value && typeof value === 'object') {
                    return self.wrapDeep(value, path.concat(key));
                }
